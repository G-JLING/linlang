 ...
 * Linlang 内建问题代码目录。
 *
 * <p>目录始终保留内建默认文本。安装语言字段后，查询结果会优先使用当前语言，
 * 语言服务尚未就绪或解析失败时仍可正常查询。</p>
 */
public final class BuiltinProblemCatalog {
    ...
    private volatile BuiltinProblemMessageKeys language;

    /**
     * 安装问题目录使用的动态语言字段。
     *
     * @param language 已绑定的语言字段；传入 {@code null} 时恢复默认文本
     */
    public void language(BuiltinProblemMessageKeys language) {
        this.language = language;
    }

    public Optional<ProblemDefinition> lookup(String code) {
        if (code == null || code.isBlank()) return Optional.empty();
        ProblemDefinition definition = DEFINITIONS.get(code.trim().toUpperCase(Locale.ROOT));
        return definition == null ? Optional.empty() : Optional.of(localize(definition));
    }
    ...
    private ProblemDefinition localize(ProblemDefinition fallback) {
        BuiltinProblemMessageKeys keys = language;
        if (keys == null) return fallback;
        try {
            String code = fallback.code();
            String description = localized(keys.descriptions.resolve().get(code), fallback.description());
            String resolution = localized(keys.resolutions.resolve().get(code), fallback.resolution());
            if (description.equals(fallback.description()) && resolution.equals(fallback.resolution())) {
                return fallback;
            }
    ...
    private static Map<String, ProblemDefinition> definitions() {
        ...
        add(values, "LIN-RUNTIME-RELOAD-FAIL", "Runtime",
                "重载时遇到了问题，一些文件未能重载。",
                "请查看失败文件和步骤，修正后重新执行软重载。");
        add(values, LANGUAGE_REFERENCE_FAILED, "File",
                "配置语言引用不存在或文本类型不匹配，相关键使用默认值。",
        ...
        add(values, VersionCheck.INVALID_CODE, "Runtime",
                ...
                "请检查版本元数据是否为 A.B.C.D 格式，并重新获取完整构建：" + VersionCheck.PROJECT_URL);
        add(values, MESSAGE_TEMPLATE_INSTALL_FAILED, "Runtime",
                "运行时内建消息模板无法安装。",
                "请检查运行时语言资源与语言服务是否已经完成初始化。");
        ...
        add(values, COMMAND_EXECUTION_FAILED, "Command",
                "已匹配命令的执行器抛出异常。",
                "请根据 command 与 sender 上下文检查命令业务代码。");
        ...
        add(values, COMMAND_GROUP_CALLBACK_FAILED, "Command",
                "命令组的公共结果回调执行失败。",
                "请根据 command、stage 与 handler 上下文检查命令组回调代码。");

        add(values, CONFIG_SAVE_FAILED, "File",
                "配置对象无法写入文件。",
                "请检查目标路径、文件权限、配置字段和值的可序列化性。");
        add(values, CONFIG_RELOAD_FAILED, "File",
                "配置文件无法重载到现有对象。",
                "请检查配置格式、迁移器和字段类型是否与代码契约一致。");
        add(values, CONFIG_BIND_FAILED, "File",
                "配置类无法完成首次绑定。",
                "请检查配置注解、构造方法、迁移器、文件格式和字段类型。");
        ...
        add(values, WATCH_CALLBACK_FAILED, "File",
                "文件热重载回调执行失败。",
                "请检查对应文件的重载逻辑，并查看 path 上下文。");
        add(values, WATCH_START_FAILED, "File",
                "文件热重载监听无法启动。",
                "请检查监听目录是否可创建，以及系统文件监听器是否可用。");
        add(values, WATCH_LOOP_FAILED, "File",
                "文件监听循环发生非预期故障。",
                "请检查 WatchService 状态、监听目录和回调线程是否仍然有效。");
